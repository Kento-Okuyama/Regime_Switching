smoothing <- function(N, Nt, O1, O2, L1, y1, y2, params, S, eta1_true) {
  ...
  for (t in 1:Nt) {
    ...
    tPr[,t,1,1] <- (gamma1 + eta1_pred[,t,]$matmul(gamma2))$sigmoid()
    ...
    W[,t,,] <- jPr2[,t,,] / mPr[,t+1,]$unsqueeze(-1)$clip(min=lEpsilon, max=1-lEpsilon)
  ...
  P_pred[,1:Nt,,] <- mPr[,2:(Nt+1),1]$unsqueeze(-1)$unsqueeze(-1) * mP[,2:(Nt+1),1,,] + mPr[,2:(Nt+1),2]$unsqueeze(-1)$unsqueeze(-1) * mP[,2:(Nt+1),2,,]
  ...
  tPr[,Nt+1,1,1] <- (gamma1 + eta1_pred[,Nt,]$matmul(gamma2) + gamma3 * eta2 + eta1_pred[,Nt,]$matmul(gamma4) * eta2)$sigmoid()
  tPr[,Nt+1,1,2] <- 1 - tPr[,Nt+1,1,1]
  ...
  # contingency table
  cTable1 <- table(factor(S[,Nt+1], levels=c(0,1)), factor(round(as.array(2 - jPr[,Nt+1,1,1])), levels=c(0,1)))
  ...
  output <- list(eta1_pred=as.array(eta1_pred), P_pred=as.array(P_pred), mPr=as.array(mPr[,2:(Nt+1),2]), sumLik=sumLikBest, sumLik_NxT=sumLikBest_NxT, cTable=cTable1, sensitivity=sensitivity1, specificity=specificity1, delta1=delta1, delta1_N=delta1_N)
  return(output)
}