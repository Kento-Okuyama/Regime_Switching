result <- function(N, Nt, O1, O2, L1, y1, y2, params, S, eta1_true) {
  ...
  tPr[,Nt+1,1,1] <- (gamma1 + eta1_pred[,Nt,]$matmul(gamma2))$sigmoid()$clip(min=sEpsilon, max=1-sEpsilon)
  ...
  output <- list(eta1_pred=as.array(eta1_pred), P_pred=as.array(P_pred$diagonal(dim1=3, dim2=4)), mPr=cbind(as.array(mPr[,2:(Nt+1),2]), as.numeric(1 - jPr[,Nt+1,1,1])), mPr_sm=cbind(as.array(mPr2[,2:(Nt+1),2]), as.numeric(1 - jPr[,Nt+1,1,1])), sumLik=sumLikBest, sumLik_NxT=sumLikBest_NxT, cTable=cTable1, sensitivity=sensitivity1, specificity=specificity1, delta1=delta1, delta1_N=delta1_N)
  return(output)
}