filtering <- function(seed, N, Nt, O1, O2, L1, y1, y2, S, eta1_true, nInit, maxIter) {
    ...
    try (silent=FALSE, {
      while (count <=3 && iter <= maxIter) {
        ...
        if (init == 1 && iter == 1) {
          ...
          } else if (sumLikBest < sumLik) {
            initBest <- init
            iterBest <- iter
            thetaBest <- as.numeric(torch_cat(theta))
            sumLikBest <- sumLik
            sumLikBest_NT <- sumLik / (N * Nt)
            AIC_best <- -2 * log(sumLikBest) + 2 * q
            BIC_best <- -2 * log(sumLikBest) + q * log(N * Nt)
          } }
  ...
  filter <- list(thetaBest=thetaBest, sumLikBest=sumLikBest, sumLikBest_NT=sumLikBest_NT, AIC_best=AIC_best, BIC_best=BIC_best, initBest=initBest, iterBest=iterBest)
  return(filter)
}