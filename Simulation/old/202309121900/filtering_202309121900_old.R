    ...
    try (silent=FALSE, {
      while (count <=3 && iter <= maxIter) {
        ...
        for (t in 1:Nt) {
          ...
          # 1. do the calculations for all t in 1:Nt in one?
          # array(B11$clone(),c(dim(matrix),Nt)) + mEta[,,1,]$clone()$matmul(B21$clone()) + array(eta2$clone()$outer(B31$clone(),c(dim(matrix),Nt))
          # 2. combine the three individual calculations in one matrix? so all jEta, all jP and so on?
        ...
        print(paste0('time_5 - time_4 = ', time_5 - time_4))
