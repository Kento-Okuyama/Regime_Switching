result <- function(N, Nt, O1, O2, L1, y1, y2, params, S, eta1_true) {
  ...
  for (t in 1:Nt) {
    ...
    jLik[,t,,] <- sEpsilon + const * jF[,t,,,,]$det()$clip(min=sEpsilon, max=ceil)**(-1) *
    ...
    # eta1_pred[,t,][which(!torch_isfinite(eta1_pred[,t,]), arr.ind=TRUE)] <- eta1_pred[,t,][which(!torch_isfinite(eta1_pred[,t,]), arr.ind=TRUE)] 
    P_pred[,t,,] <- mPr[,t,1]$unsqueeze(-1)$unsqueeze(-1) * mP[,t,1,,] + mPr[,t,2]$unsqueeze(-1)$unsqueeze(-1) * mP[,t,2,,]
    # P_pred[,t,,][which(!torch_isfinite(P_pred[,t,,]), arr.ind=TRUE)] <- P_pred[,t-1,,][which(!torch_isfinite(P_pred[,t,,]), arr.ind=TRUE)] 
  ...
  # eta1_pred[,Nt,][which(!torch_isfinite(eta1_pred[,Nt,]), arr.ind=TRUE)] <- eta1_pred[,Nt-1,][which(!torch_isfinite(eta1_pred[,Nt,]), arr.ind=TRUE)] 
  ...
  # P_pred[,Nt,,][which(!torch_isfinite(P_pred[,Nt,,]), arr.ind=TRUE)] <- P_pred[,Nt-1,,][which(!torch_isfinite(P_pred[,Nt,,]), arr.ind=TRUE)]
  ...
  # eta1_pred[,Nt+1,][which(!torch_isfinite(eta1_pred[,Nt+1,]), arr.ind=TRUE)] <- eta1_pred[,Nt,][which(!torch_isfinite(eta1_pred[,Nt+1,]), arr.ind=TRUE)] 
  ...
  # P_pred[,Nt+1,,][which(!torch_isfinite(P_pred[,Nt+1,,]), arr.ind=TRUE)] <- P_pred[,Nt,,][which(!torch_isfinite(P_pred[,Nt+1,,]), arr.ind=TRUE)] 
  ...
  output <- list(eta1_pred=as.array(eta1_pred), P_pred=as.array(P_pred), mPr=as.array(mPr[,2:(Nt+1),2]), mPr_sm=as.array(mPr2[,2:(Nt+1),2]), sumLik=sumLikBest, sumLik_NxT=sumLikBest_NxT, cTable=cTable1, sensitivity=sensitivity1, specificity=specificity1, delta1=delta1, delta1_N=delta1_N)
  return(output)
}