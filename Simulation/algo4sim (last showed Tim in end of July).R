  ...
  try (silent=FALSE, {
    while (count <=3 && iter <= maxIter) {
      ...
      for (t in 1:Nt) {
        ...
        W[,t,1,1] <- jPr2[,t,1,1]$clone() / (1 - mPr[,t+1]$clone())$clip(min=sEpsilon)
        W[,t,2,1] <- jPr2[,t,2,1]$clone() / mPr[,t+1]$clone()$clip(min=sEpsilon)
        W[,t,2,2] <- jPr2[,t,2,2]$clone() / mPr[,t+1]$clone()$clip(min=sEpsilon)
        W[,t,,]$clip_(max=ceil)
...
for (h in 1:H) {
  ...
  jDelta[,Nt+h,1,1,] <- eta1[,Nt+h,]$clone() - jEta[,Nt+h,1,1,]$clone() 
  jDelta[,Nt+h,2,1,] <- eta1[,Nt+h,]$clone() - jEta[,Nt+h,2,1,]$clone() 
  jDelta[,Nt+h,2,2,] <- eta1[,Nt+h,]$clone() - jEta[,Nt+h,2,2,]$clone() 
  ...
  jV[,Nt+h,1,1,] <- y1[,Nt+h,]$clone() - (d + jEta[,Nt+h,1,1,]$clone()$matmul(Lmd$transpose(1, 2))) 
  jV[,Nt+h,2,1,] <- y1[,Nt+h,]$clone() - (d + jEta[,Nt+h,2,1,]$clone()$matmul(Lmd$transpose(1, 2))) 
  jV[,Nt+h,2,2,] <- y1[,Nt+h,]$clone() - (d + jEta[,Nt+h,2,2,]$clone()$matmul(Lmd$transpose(1, 2))) 
  
  jF[,Nt+h,1,1,,] <- Lmd$matmul(jP[,Nt+h,1,1,,]$clone())$matmul(Lmd$transpose(1, 2)) + R1
  jF[,Nt+h,2,1,,] <- Lmd$matmul(jP[,Nt+h,2,1,,]$clone())$matmul(Lmd$transpose(1, 2)) + R2
  jF[,Nt+h,2,2,,] <- Lmd$matmul(jP[,Nt+h,2,2,,]$clone())$matmul(Lmd$transpose(1, 2)) + R2
  
  if (t == 1) {tPr[,Nt+h,1] <- gamma11$sigmoid() 
  } else {tPr[,Nt+h,1] <- (gamma11 + eta1[,t-1,]$clone()$matmul(gamma21))$sigmoid() }
  
  jPr[,Nt+h,1,1] <- (1-tPr[,Nt+h,1]$clone()) * (1-mPr[,t]$clone())
  jPr[,Nt+h,2,1] <- tPr[,Nt+h,1]$clone() * (1-mPr[,t]$clone()) 
  jPr[,Nt+h,2,2] <- mPr[,t]$clone() 
}