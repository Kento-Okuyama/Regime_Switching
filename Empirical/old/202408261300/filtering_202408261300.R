filtering <- function(seed, N, Nt, O1, O2, L1, y1, y2, init, maxIter) {
  ...
  mP_DO <- torch_tensor(sEpsilon, requires_grad=FALSE)
  ...
  filter <- list(init_best=init_best, iter_best=iter_best, theta_best=theta_best, sumLik_best_NT=sumLik_best_NT, AIC_best=AIC_best, BIC_best=BIC_best, output_best=output_best, mPr_best=mPr_best, theta_list=data.table(theta_list_arranged), output_list=output_list)
  gc()
  return(filter)
}
