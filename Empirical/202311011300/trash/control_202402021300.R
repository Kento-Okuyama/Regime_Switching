...
control <- function(init) {
  ...
  process <- function() {
    df <- preprocessing()
    saveRDS(df, paste('output/df__emp_', seed, '_N_', df$N, '_T_', df$Nt, '_O1_', df$O1, '_O2_', df$O2, '_L1_', df$L1,'.RDS', sep=''))
    ...
    saveRDS(filter, paste('output/filter__emp_', seed, '_N_', df$N, '_T_', df$Nt, '_O1_', df$O1, '_O2_', df$O2, '_L1_', df$L1, '_init_', init, '.RDS', sep=''))
...
# control()
# filtered <- readRDS(paste('output/filter__emp_42_N_80_T_51_O1_17_O2_3_L1_7_init_1.RDS', sep=''))
# df <- readRDS(paste('output/df__emp_42_N_80_T_51_O1_17_O2_3_L1_7.RDS', sep=''))
