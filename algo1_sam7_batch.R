...
y <- y[schuffled,,]
eta <- eta[schuffled,,]
# split samples into two groups (mini-batches)
  ...
  while (count < 3) {
    ...
    for (t in 1:Nt) { 
      ...
      for (js in 1:nrow(jS)) {
        ...
        for (noNaRow in noNaRows[[t]]) {jV[noNaRow,t,s1,s2,] <- y[noNaRow,t,] - (k[[s1]] + torch_matmul(torch_clone(jEta[noNaRow,t,s1,s2,]), Lmd[[s1]]))} # Eq.5
