...
for (init in 1:nInit) {
  ...
  Q1 <- torch_tensor(diag(Nf1))
  Q2 <- torch_tensor(diag(Nf1))
  R1 <- torch_tensor(diag(No1))
  R2 <- torch_tensor(diag(No1))
  gamma11 <- torch_tensor(-2.95)
  gamma21 <- torch_tensor(rep(0, Nf1))
  tau11 <- torch_tensor(1)
  ...
  with_detect_anomaly ({
    ...
    while (count <=3 && iter <= maxIter) {
      ...
      tau11$requires_grad_()
      
      theta <- list(B11=B11, B12=B12, B21=B21, B22=B22, B31=B31, B32=B32,
                    Q1=Q1, Q2=Q2, R1=R1, R2=R2, gamma11=gamma11, gamma21=gamma21, tau11=tau11)
      ...
      tau11 <- torch_tensor(theta$tau11)
