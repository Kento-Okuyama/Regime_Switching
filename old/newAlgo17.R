      ...
      for (t in 1:Nt) { 
        ...
        for (s1 in 1:2) {
          ...
          with_no_grad ({ 
            ...
            jP[,t,s1,s2,,] <- jPEig[[2]]$real$matmul(jPEig[[1]]$real$diag_embed())$matmul(jPEig[[2]]$real$transpose(2,3)) })
          ...
          with_no_grad ({
            ...
            jF[,t,s1,s2,,] <- jFEig[[2]]$real$matmul(jFEig[[1]]$real$diag_embed())$matmul(jFEig[[2]]$real$transpose(2, 3)) }) 

          # kalman gain function
          KG[,t,s1,,,] <- jP[,t,s1,,,]$clone()$matmul(Lmd$transpose(1, 2))$matmul(jF[,t,s1,,,]$clone()$cholesky_inverse())
          ...
          for (s2 in 1:2) {
            ...
            with_no_grad ({
              ...
              jP2[,t,s1,s2,,] <- jP2Eig[[2]]$real$matmul(jP2Eig[[1]]$real$diag_embed())$matmul(jP2Eig[[2]]$real$transpose(2, 3)) }) 
            
            # joint likelihood f(eta_{t}|s,s',eta_{t-1})
            # Eq.12
            if (s1 == 2 || s2 == 1) {
              jLik[,t,s1,s2] <- (2*pi)**(-Nf1/2) * jP[,t,s1,s2,,]$clone()$det()**(-1) * 
                (-.5 * jDelta[,t,s1,s2,]$clone()$unsqueeze(dim=2)$matmul(jP[,t,s1,s2,,]$clone()$cholesky_inverse())$matmul(jDelta[,t,s1,s2,]$clone()$unsqueeze(dim=-1))$squeeze()$squeeze())$exp()
        ...
        with_no_grad ({
          mPEig <- linalg_eigh(mP[,t+1,,,]) 
          mPEig[[1]]$real$clip_(lEpsilon, ceil)
          mP[,t+1,,,] <- mPEig[[2]]$real$matmul(mPEig[[1]]$real$diag_embed())$matmul(mPEig[[2]]$real$transpose(3, 4)) }) }
