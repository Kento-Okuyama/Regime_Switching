...
m1 <- cbind(melt(apply(as.array(mEta[,2:(Nt+1),,1]), c(2,3), mean)), melt(apply(as.array(eta1[,,1]), 2, mean)))
...
m2 <- cbind(melt(apply(as.array(mEta[,2:(Nt+1),,2]), c(2,3), mean)), melt(apply(as.array(eta1[,,2]), 2, mean)))
...
plot8 <- ggplot(data=melt(apply(as.array(mP[,2:(Nt+1),,1,1]), c(2,3), mean)), aes(X1, value, group=X2, color=X2)) + geom_line() + theme(legend.position='none')
plot9 <- ggplot(data=melt(apply(as.array(mP[,2:(Nt+1),,2,2]), c(2,3), mean)), aes(X1, value, group=X2, color=X2)) + geom_line() + theme(legend.position='none')

plot_grid(plot4, plot5, plot8, plot9)
