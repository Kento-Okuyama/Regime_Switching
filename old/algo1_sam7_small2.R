...
for (init in 1:nInit) {
  ...
  KG <- torch_full(c(N,Nt,2,2,Nf,No), NaN)
  KGLmd <- I_KGLmd <- torch_full(c(N,Nt,2,2,Nf,Nf), NaN)
  denom1 <- denom12 <- denom2 <- denom22 <-torch_full(c(N,Nt+1), NaN)
  subEta <- torch_full(c(N,Nt,2,2,Nf), NaN)
  subEtaSq <- subEtaSqsym <- subEtaSqE <- torch_full(c(N,Nt,2,2,Nf,Nf), NaN)
  ...
  for (iter in 1:nIter) {
    ...
    for (t in 1:Nt) { 
      ...
      for (js in 1:nrow(jS)) {
        ...
        jDelta[,t,s1,s2,] <- eta[,t,] - torch_clone(jEta[,t,s1,s2,]) # Eq.3
        ...
        jPE[,t,s1,s2,,] <- torch_clone(jPsym[,t,s1,s2,,]) + epsD * torch_eye(Nf) # add a small constant to ensure p.s.d.

        jV[,t,s1,s2,] <- y[,t,] - (k[[s1]] + torch_matmul(torch_clone(jEta[,t,s1,s2,]), Lmd[[s1]])) # Eq.5
        ...
        jFE[,t,s1,s2,,] <- torch_clone(jFsym[,t,s1,s2,,]) + epsD * torch_eye(No) # add a small constant to ensure p.s.d.

        for (i in 1:N) {
          # kalman gain function
          KG[i,t,s1,s2,,] <- torch_matmul(torch_matmul(torch_clone(jPE[i,t,s1,s2,,]), Lmd[[s1]]), torch_inverse(torch_clone(jFE[i,t,s1,s2,,])))
          jEta2[i,t,s1,s2,] <- torch_clone(jEta[i,t,s1,s2,]) + torch_matmul(torch_clone(KG[i,t,s1,s2,,]), torch_clone(jV[i,t,s1,s2,])) # Eq.7
          KGLmd[i,t,s1,s2,,] <- torch_matmul(torch_clone(KG[i,t,s1,s2,,]), torch_transpose(Lmd[[s1]], 1, 2))
          I_KGLmd[i,t,s1,s2,,] <- torch_eye(Nf) - torch_clone(KGLmd[i,t,s1,s2,,])
          
          # jP2[i,t,s1,s2,,] <- torch_matmul(I_KGLmd, jP[i,t,s1,s2,,])} # Eq.8 
          jP2[i,t,s1,s2,,] <- torch_matmul(torch_matmul(torch_clone(I_KGLmd[i,t,s1,s2,,]), torch_clone(jPE[i,t,s1,s2,,])), torch_transpose(torch_clone(I_KGLmd[i,t,s1,s2,,]), 1, 2)) + torch_matmul(torch_matmul(torch_clone(KG[i,t,s1,s2,,]), R[[s1]]), torch_transpose(torch_clone(KG[i,t,s1,s2,,]), 1, 2)) # Eq.9
          jP2E[i,t,s1,s2,,] <- torch_clone(jP2[i,t,s1,s2,,]) + epsD * torch_eye(Nf) } # add a small constant to ensure p.s.d. 
        ...
        for (i in 1:N) {
          jLik[i,t,s1,s2] <- 
            (-.5*pi)**(-Nf/2) * torch_det(torch_clone(jPE[i,t,s1,s2,,]))**(-1) * 
            torch_exp(-.5*torch_matmul(torch_matmul(torch_clone(jDelta[i,t,s1,s2,]), torch_inverse(torch_clone(jPE[i,t,s1,s2,,]))), torch_clone(jDelta[i,t,s1,s2,]))) } } # Eq.12
      ...
      for (i in 1:N) {
        mLik[i,t] <- torch_sum(torch_clone(jLik[i,t,,]) * torch_clone(jPr[i,t,,]))
        # (updated) joint probability P(s,s'|eta_{t})
        jPr2[i,t,,] <- torch_clone(jLik[i,t,,]) * torch_clone(jPr[i,t,,]) / (torch_clone(mLik[i,t]) + epsilon) }
      mPr[,t+1] <- torch_sum(torch_clone(jPr2[,t,2,]), dim=2)
      
      # step 11: collapsing procedure
      print('      Collapsing')
      for (s2 in 1:2) { 
        denom1[,t+1] <- 1 - torch_clone(mPr[,t+1])
        denom12[,t+1] <- torch_clone(denom1[,t+1]) + epsilon
        W[,t,1,s2] <- torch_clone(jPr2[,t,1,s2]) / torch_clone(denom12[,t+1])
        denom2[,t+1] <- torch_clone(mPr[,t+1])
        denom22[,t+1] <- torch_clone(denom2[,t+1]) + epsilon
        W[,t,2,s2] <- torch_clone(jPr2[,t,2,s2]) / torch_clone(denom22[,t+1]) }
      
      for (f in 1:Nf) {mEta[,t+1,,f] <- torch_sum(torch_clone(W[,t,,]) * torch_clone(jEta2[,t,,,f]), dim=3)}
      
      for (s2 in 1:2) {subEta[,t,,s2,] <- torch_clone(mEta[,t+1,,]) - torch_clone(jEta2[,t,,s2,])}
      subEtaSq[,t,,,,] <- torch_matmul(torch_unsqueeze(torch_clone(subEta[,t,,,]), dim=-1), torch_unsqueeze(torch_clone(subEta[,t,,,]), dim=-2))
      subEtaSqsym[,t,,,,] <- (torch_clone(subEtaSq[,t,,,,]) + torch_transpose(torch_clone(subEtaSq[,t,,,,]), 4, 5)) / 2 # ensure symmetry

      for (js in 1:nrow(jS)) {
        s1 <- jS$s1[js]
        s2 <- jS$s2[js]
        subEtaSqE[,t,s1,s2,,] <- torch_clone(subEtaSqsym[,t,s1,s2,,]) + epsD * torch_eye(Nf)} 
      ...
      for (jnf in 1:nrow(jNf)) {
        ...
        mP[,t+1,,f1,f2] <- torch_sum(torch_clone(W[,t,,]) * (torch_clone(jP2E[,t,,,f1,f2]) + torch_clone(subEtaSqE[,t,,,f1,f2])), dim=3) }
      mPsym[,t+1,,,] <- (torch_clone(mP[,t+1,,,]) + torch_transpose(torch_clone(mP[,t+1,,,]), 3, 4)) / 2 # ensure symmetry
      
      for (s1 in 1:2) {mPE[,t+1,s1,,] <- torch_clone(mPsym[,t+1,s1,,]) + epsD * torch_eye(Nf)} } # add a small constant to ensure p.s.d.
