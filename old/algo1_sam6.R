...
for (init in 1:nInit) {
  ...
  jPsym <- torch_full(c(N,Nt,2,2,Nf,Nf), NaN) # Eq.4 (LHS)
  jPE <- torch_full(c(N,Nt,2,2,Nf,Nf), NaN) # Eq.4 (LHS)
  ...
  jFsym <- torch_full(c(N,Nt,2,2,Nf,Nf), NaN) # Eq.4 (LHS)
  jFE <- torch_full(c(N,Nt,2,2,Nf,Nf), NaN) # Eq.4 (LHS)
  ...
  jP2E <- torch_full(c(N,Nt,2,2,Nf,Nf), NaN) # Eq.8 (LHS)
  mEta <- torch_full(c(N,Nt+1,2,Nf), NaN) # Eq.9-1 (LHS)
  mP <- torch_full(c(N,Nt+1,2,Nf,Nf), NaN) # Eq.9-2 (LHS)
  mPE <- torch_full(c(N,Nt+1,2,Nf,Nf), NaN) # Eq.9-2 (LHS)
  ...
  for (iter in 1:nIter) {
    ...
    for (t in 1:Nt) { 
      ...
      for (js in 1:nrow(jS)) {
        ...
        if (sum(as.numeric(torch_det(jPsym[,t,s1,s2,,])) < epsilon) > 0) {
          jPInd <- which(as.numeric(torch_det(jPsym[,t,s1,s2,,])) < epsilon)
          for (ind in jPInd) {jPE[ind,t,s1,s2,,] <- torch_clone(jPsym[ind,t,s1,s2,,]) + epsD * torch_eye(Nf)} } # add a small constant to ensure p.s.d.
        if (sum(as.numeric(torch_det(jPsym[,t,s1,s2,,])) >= epsilon) > 0) {
          jPNInd <- which(as.numeric(torch_det(jPsym[,t,s1,s2,,])) >= epsilon)
          for (ind in jPNInd) {jPE[ind,t,s1,s2,,] <- torch_clone(jPsym[ind,t,s1,s2,,])} } 
        jPChol[,t,s1,s2,,] <- torch_cholesky(jPE[,t,s1,s2,,], upper=FALSE) # Cholesky decomposition
        ...
        if (sum(as.numeric(torch_det(jFsym[,t,s1,s2,,])) < epsilon) > 0) {
          jFInd <- which(as.numeric(torch_det(jFsym[,t,s1,s2,,])) < epsilon)
          for (ind in jFInd) {jFE[ind,t,s1,s2,,] <- torch_clone(jFsym[ind,t,s1,s2,,]) + epsD * torch_eye(No)} # add a small constant to ensure p.s.d.
        if (sum(as.numeric(torch_det(jFsym[,t,s1,s2,,])) >= epsilon) > 0) {
          jFNInd <- which(as.numeric(torch_det(jFsym[,t,s1,s2,,])) < epsilon)
          for (ind in jFNInd) {jFE[ind,t,s1,s2,,] <- torch_clone(jFsym[ind,t,s1,s2,,])} } 
        jFChol[,t,s1,s2,,] <- torch_cholesky(jFE[,t,s1,s2,,], upper=FALSE) # Cholesky decomposition
        print('ok')
        ...
        for (noNaRow in noNaRows[[t]]) {
          ...
          jP2[noNaRow,t,s1,s2,,] <- torch_matmul(torch_matmul(I_KGLmd, jPE[noNaRow,t,s1,s2,,]), torch_transpose(I_KGLmd, 1, 2)) + torch_matmul(torch_matmul(KG, R[[s1]]), torch_transpose(KG, 1, 2)) # Eq.9
        ...
        for (naRow in naRows[[t]]) {
          ...
          jP2E[naRow,t,s1,s2,,] <- jP[naRow,t,s1,s2,,] } # Eq.8 (for missing entries)
        ...
        for (noNaRow in noNaRows[[t]]) {
          jLik[noNaRow,t,s1,s2] <- 
            (-.5*pi)**(-Nf/2) * torch_prod(torch_diag(jPChol[noNaRow,t,s1,s2,,]))**(-1) * torch_exp(-.5 * torch_matmul(torch_matmul(jDelta[noNaRow,t,s1,s2,], torch_cholesky_inverse(jPChol[noNaRow,t,s1,s2,,], upper=FALSE)), jDelta[noNaRow,t,s1,s2,])) } } # Eq.12
      ...
      for (s2 in 1:2) { 
        denom1 <- 1 - mPr[,t+1]
        denom12[denom1==0] <- denom1[denom1==0] + epsilon
        denom12[denom1>0] <- denom1[denom1>0] 
        W[,t,1,s2] <- jPr2[,t,1,s2] / denom12
        denom2 <- mPr[,t+1]
        denom22[denom2==0] <- denom2[denom2==0] + epsilon
        denom22[denom2>0] <- denom2[denom2>0]
        W[,t,2,s2] <- jPr2[,t,2,s2] / denom22 }
      
      for (f in 1:Nf) {mEta[,t+1,,f] <- torch_sum(W[,t,,] * jEta2[,t,,,f], dim=3)}
      
      subEta <- torch_full_like(jEta2[,t,,,], NaN)
      for (s2 in 1:2) {subEta[,,s2,] <- mEta[,t+1,,] - jEta2[,t,,s2,]}
      subEta1 <- torch_unsqueeze(subEta, dim=-1)
      subEta2 <- torch_unsqueeze(subEta, dim=-2)
      subEtaSq <- torch_matmul(subEta1, subEta2)
      subEtaSqsym <- (torch_clone(subEtaSq) + torch_transpose(torch_clone(subEtaSq), 4, 5)) / 2 # ensure symmetry
      
      for (js in 1:nrow(jS)) {
        s1 <- jS$s1[js]
        s2 <- jS$s2[js]
        if (sum(as.numeric(torch_det(subEtaSqsym[,s1,s2,,])) < epsilon) > 0) {
          subEtaSqInd <- which(as.numeric(torch_det(subEtaSqsym[,s1,s2,,])) < epsilon)
          for (ind in subEtaSqInd) {subEtaSqE[ind,s1,s2,,] <- torch_clone(subEtaSqsym[ind,s1,s2,,]) + epsD * torch_eye(Nf)} }
        if (sum(as.numeric(torch_det(subEtaSqsym[,s1,s2,,])) >= epsilon) > 0) {
          subEtaSqNInd <- which(as.numeric(torch_det(subEtaSqsym[,s1,s2,,])) >= epsilon)
          for (ind in subEtaSqNInd) {subEtaSqE[ind,s1,s2,,] <- torch_clone(subEtaSqsym[ind,s1,s2,,])} } } 

      jNf <- expand.grid(f1=1:Nf, f2=1:Nf)
      for (jnf in 1:nrow(jNf)) {
        f1 <- jNf$f1[jnf]
        f2 <- jNf$f2[jnf] 
        mP[,t+1,,f1,f2] <- torch_sum(W[,t,,] * (jP2E[,t,,,,] + subEtaSqE)[,,,f1,f2], dim=3) }
      mPsym[,t+1,,,] <- (torch_clone(mP[,t+1,,,]) + torch_transpose(torch_clone(mP[,t+1,,,]), 3, 4)) / 2 # ensure symmetry
      
      for (s1 in 1:2) {
        if (sum(as.numeric(torch_det(mPsym[,t+1,s1,,])) < epsilon) > 0) {
          mPInd <- which(as.numeric(torch_det(mPsym[,t+1,s1,,])) < epsilon)
          for (ind in mPInd) {mPE[ind,t+1,s1,,] <- torch_clone(mPsym[ind,t+1,s1,,]) + epsD * torch_eye(Nf)} } 
        if (sum(as.numeric(torch_det(mPsym[,t+1,s1,,])) >= epsilon) > 0) {
          mPNInd <- which(as.numeric(torch_det(mPsym[,t+1,s1,,])) >= epsilon)
          for (ind in mPNInd) {mPE[ind,t+1,s1,,] <- torch_clone(mPsym[ind,t+1,s1,,])} } } } # add a small constant to ensure p.s.d.
