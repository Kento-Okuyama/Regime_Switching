...
for (init in 1:nInit) {
  ...
  a1 <- torch_tensor(torch_normal(size=Nf), requires_grad=TRUE) 
  a2 <- torch_tensor(torch_normal(size=Nf), requires_grad=TRUE) 
  a <- list(a1, a2)
  B1d <- torch_tensor(torch_normal(size=Nf), requires_grad=TRUE)
  B2d <- torch_tensor(torch_normal(size=Nf), requires_grad=TRUE)
  B1 <- torch_diag(B1d)
  B2 <- torch_diag(B2d)
  B <- list(B1, B2)
  k1 <- torch_tensor(torch_normal(size=No), requires_grad=TRUE)
  k2 <- torch_tensor(torch_normal(size=No), requires_grad=TRUE)
  k <- list(k1, k2)
  Lmd1v <- torch_tensor(torch_normal(size=No), requires_grad=TRUE)
  Lmd2v <- torch_tensor(torch_normal(size=No), requires_grad=TRUE)
  ...
  Lmd <- list(Lmd1, Lmd2)
  alpha1 <- torch_tensor(torch_normal(size=1), requires_grad=TRUE)
  alpha2 <- torch_tensor(torch_normal(size=1), requires_grad=TRUE)
  alpha <- list(alpha1, alpha2)
  beta1 <- torch_tensor(torch_normal(size=Nf), requires_grad=TRUE)
  beta2 <- torch_tensor(torch_normal(size=Nf), requires_grad=TRUE)
  beta <- list(beta1, beta2)
  Q1d <- torch_tensor(torch_normal(size=Nf)**2 + epsilon, requires_grad=TRUE)
  Q2d <- torch_tensor(torch_normal(size=Nf)**2 + epsilon, requires_grad=TRUE)
  Q1 <- torch_diag(Q1d)
  Q2 <- torch_diag(Q2d)
  Q <- list(Q1, Q2)
  R1d <- torch_tensor(torch_normal(size=No)**2 + epsilon, requires_grad=TRUE)
  R2d <- torch_tensor(torch_normal(size=No)**2 + epsilon, requires_grad=TRUE)
  R1 <- torch_diag(R1d)
  R2 <- torch_diag(R2d)
  R <- list(R1, R2)
  ...
  for (iter in 1:nIter) {
    ...
    for (t in 1:Nt) { 
      ...
      for (js in 1:nrow(jS)) {
        ...
        while (sum(as.numeric(torch_det(jP[,t,s1,s2,,])) <= 0) > 0) {
          jPInd <- as.numeric(torch_det(jP[,t,s1,s2,,])) <= 0
          for (ind in jPInd) {jP[ind,t,s1,s2,,] <- jP[ind,t,s1,s2,,] + epsilon * torch_eye(Nf)} } # add a small constant to ensure p.s.d.
        ...
        while (sum(as.numeric(torch_det(jF[,t,s1,s2,,])) <= 0) > 0) {
          jFInd <- as.numeric(torch_det(jF[,t,s1,s2,,])) <= 0
          for (ind in jFInd) {jF[ind,t,s1,s2,,] <- jF[ind,t,s1,s2,,] + epsilon * torch_eye(No)} } # add a small constant to ensure p.s.d.
      ...
      for (js in 1:nrow(jS)) {
        s1 <- jS$s1[js]
        s2 <- jS$s2[js]
        while (sum(as.numeric(torch_det(subEtaSq[,s1,s2,,])) <= 0) > 0) {
          subEtaSqInd <- as.numeric(torch_det(subEtaSq[,s1,s2,,])) <= 0
          for (ind in subEtaSqInd) {
            print(paste0('   (i,t,s1,s2): (', ind, ',', s1, ',', s2, '): ', 'det(subEtaSq[i,s1,s2,,]) = ', as.numeric(torch_det(subEtaSq[ind,s1,s2,,]))))
            subEtaSq[ind,s1,s2,,] <- subEtaSq[ind,s1,s2,,] + epsilon * torch_eye(Nf)} } } # add a small constant to ensure p.s.d.
      ...
      for (s1 in 1:2) {
        while (sum(as.numeric(torch_det(mP[,t+1,s1,,])) <= 0) > 0) {
          mPInd <- as.numeric(torch_det(mP[,t+1,s1,,])) <= 0
          for (ind in mPInd) {
            print(paste0('   (i,t,s1): (', ind, ',', s1, '): ', 'det(mP[,t+1,s1,,]) = ', as.numeric(torch_det(mP[ind,t+1,s1,,]))))
            mP[ind,t+1,s1,,] <- mP[ind,t+1,s1,,] + epsilon * torch_eye(Nf)} } } } # add a small constant to ensure p.s.d.
