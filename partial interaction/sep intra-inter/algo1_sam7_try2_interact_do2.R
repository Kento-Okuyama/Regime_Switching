...
eta2 <- eta3D2
Nf2 <- dim(eta2)[3]
eta2Mean <- colMeans(eta2, na.rm=TRUE)
eta2Sd <- sd(eta2)
      ...
      for (t in 1:Nt) { 
        ...
        for (js in 1:nrow(jS)) {
          ...
          with_no_grad ({
            ...
            while (sum(as.numeric(torch_det(jF[,t,s1,s2,,])) < epsilon) > 0) {
              ...
              for (ind in jFInd) {jF[ind,t,s1,s2,,]$add_(5e-1 * torch_eye(No1))} } }) # add a small constant to ensure p.s.d.
          ...
          if (length(naRows[[t]]) == N) {
            ...
            for (noNaRow in noNaRows[[t]]) {
              ...
              with_no_grad ({
                ...
                if (as.numeric(torch_det(jP2[noNaRow,t,s1,s2,,])) < epsilon) {jP2[noNaRow,t,s1,s2,,]$add_(3e-1 * torch_eye(Nf1)) } }) } } # add a small constant to ensure p.s.d.
        ...
        if (t == 1) {
          ...
          if (length(noNaRows[[t-1]]) == N) {
            ...
            tPr[,t,2] <- 1 # torch_sigmoid(torch_squeeze(alpha[[2]]) + torch_matmul(torch_tensor(eta12[,t-1,]), beta[[2]])) 
            ...
            for (noNaRow in noNaRows[[t-1]]) {
              ...
              tPr[noNaRow,t,2] <- 1 # torch_sigmoid(torch_squeeze(alpha[[2]]) + torch_matmul(torch_tensor(eta12[noNaRow,t-1,]), beta[[2]])) 
        ...
        with_no_grad({ 
          ...
          for (js in 1:nrow(jS)) {
            ...
            while (sum(as.numeric(torch_det(subEtaSq[,s1,s2,,])) < epsilon) > 0) {
              ...
              for (ind in subEtaSqInd) {subEtaSq[ind,s1,s2,,]$add_(3e-1 * torch_eye(Nf1))} } } })  
