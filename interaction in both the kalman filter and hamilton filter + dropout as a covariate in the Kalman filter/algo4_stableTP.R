...
for (init in 1:nInit) {
  ...
  try (silent = TRUE, {
    while (count < 3 && iter < 30) {
      ...
      R2 <- torch_diag(R2d)
      R <- list(R1, R2)
      theta <- list(a1=a1, a2=a2, B1d=B1d, B2d=B2d, C1d=C1d, C2d=C2d, D1=D1, D2=D2, k1=k1, k2=k2, Lmd1v=Lmd1v, Lmd2v=Lmd2v, Omega1v=Omega1v, Omega2v=Omega2v, M1=M1, M2=M2, alpha1=alpha1, alpha2=alpha2, gamma1=gamma1, gamma2=gamma2, Q1d=Q1d, Q2d=Q2d, R1d=R1d, R2d=R2d)
      ...
      with_no_grad({
        result <- adam5(loss=loss, theta=theta, m=m, v=v)
        theta <- result$theta
        m <- result$m 
        v <- result$v 
